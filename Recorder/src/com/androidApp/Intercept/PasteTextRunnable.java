...
public class PasteTextRunnable implements Runnable {
	...
	public void run() {
        ...
        pasteValues(v, classCount, pasteViewDirectives);
	}
	
	public void pasteValues(View v, ClassCount classCount, List<ViewDirective> pasteViewDirectives) {
		if (v instanceof EditText) {
			for (ViewDirective viewDirective : pasteViewDirectives) {
				if (viewDirective.match(v, classCount.mCount, ViewDirective.ViewOperation.PASTE_TEXT,  ViewDirective.When.ON_ACTIVITY_START)) {
			...
			for (int i = 0; i < vg.getChildCount(); i++) {
				...
				pasteValues(vChild, classCount, pasteViewDirectives);
			}
		}	
	}	
}
