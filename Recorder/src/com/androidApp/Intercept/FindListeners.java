...
public class FindListeners {
	 ...
	 */
	public static boolean hasListener(View v) {
		try {
			...
			if (v instanceof NumberPicker) {
				if (ListenerIntercept.getValueChangeListener((NumberPicker) v) != null) {
					return true;
				}
			}
	 ...
	 */
	public static View findViewWithListener(View v) {
		return findViewWithListener(v, v.getRootView());
	}
	
	// internal worker function for above.
	private static View findViewWithListener(View v, View vRoot) {
		if (v == vRoot) {
			return null;
		} else if (hasListener(v)) {
			return v;
		} else {
			ViewParent vp = v.getParent();
			if (vp instanceof View) {
				return findViewWithListener((View) vp, vRoot);
			}
		}
		return null;
	}
